...
export default function CartDrawer({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    return (
        <>
            ...
            {/* Drawer */}
            <div 
                className={`fixed top-0 right-0 h-full w-full sm:w-[450px] bg-white z-[70] shadow-2xl transition-transform duration-500 ease-in-out transform flex flex-col ${isOpen ? "translate-x-0" : "translate-x-full"}`}
            >
                {/* Header */}
                ...
                {items.length > 0 && (
                    <div className="p-8 border-t border-gray-100 bg-white">
                        ...
                        <Link href="/cart" onClick={onClose} className="block w-full">
                            <button className="w-full bg-black text-white text-[10px] uppercase tracking-[0.3em] font-medium py-5 hover:bg-brand-primary transition-all duration-500 shadow-lg hover:shadow-brand-primary/10">
                                View Cart & Checkout
                            </button>
                        </Link>
                    </div>
